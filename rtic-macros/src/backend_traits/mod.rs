...
/// of different RTIC compilation Passes
pub trait RticBackendBase {
    // fn enable_interrupt(&self, device: syn::Path) -> TokenStream2;

    // fn disable_interrupt(&self, device: syn::Path) -> TokenStream2;

    // fn pend_interrupt(&self, device: syn::Path, handler: syn::Ident) -> TokenStream2;

    // fn unpend_interrupt(&self, device: syn::Path, handler: syn::Ident) -> TokenStream2;
...
/// Interface for providing the low-level hardware bindings specific for a target (A.k.a The Backend) to be used during the preprocessing and code generation phases
/// of the **Hw tasks Compilation Pass*.
pub trait HwPassBackend: RticBackendBase {
    ...
    fn pre_init(&self, app: &App, analysis: &Analysis) -> Vec<TokenStream2>;
    ...
    /// ## Note
    /// This function will be called several times in case of a multicore system, each time with different `app_info` and `app_analysis`.
    fn post_init(&self, app: &App, analysis: &Analysis) -> Vec<TokenStream2>;
...
pub trait SwPassBackend: RticBackendBase {
    ...
    fn check_stack_overflow_before_init(&self, app: &App, analysis: &Analysis)
        -> Vec<TokenStream2>;
